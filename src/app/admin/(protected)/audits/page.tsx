...
export default async function AdminAuditsPage() {
  const [projects, audits] = await Promise.all([
    ...
    getDb().performanceAudit.findMany({ take: 30, orderBy: { createdAt: "desc" }, select: { id: true, testedUrl: true, strategy: true, status: true, performanceScore: true, accessibilityScore: true, bestPracticesScore: true, seoScore: true, auditedAt: true, createdAt: true, failureMessage: true, source: true, project: { select: { title: true, slug: true } } } }),
  ]);
  const configured = Boolean(process.env.GOOGLE_PAGESPEED_API_KEY);

  return <main className="admin-main">
    <div className="admin-page-heading"><div><p className="admin-eyebrow">Measured evidence</p><h1>Performance audits</h1><p>Run dated Google PageSpeed checks and keep every successful or failed result attached to its project.</p></div></div>
    ...
    <section className="admin-audit-history"><div className="admin-panel-heading"><div><p className="admin-eyebrow">History</p><h2>Recent audit runs</h2></div></div>
      {audits.length ? <div className="admin-audit-list">{audits.map((audit) => <article key={audit.id}>
        <header><div><span className={`admin-status admin-audit-status-${audit.status.toLowerCase()}`}>{audit.status}</span><p>{audit.strategy.toLowerCase()} · {audit.auditedAt ? audit.auditedAt.toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" }) : audit.createdAt.toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</p></div><Link href={`/projects/${audit.project.slug}`} target="_blank">View project ↗</Link></header>
