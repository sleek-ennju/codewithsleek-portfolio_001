  ...
  return (
    <main className="admin-main">
      <div className="admin-page-heading">
        <div>
          <p className="admin-eyebrow">Content system</p>
          <h1>Testimonials</h1>
          <p>Publish concise, attributable client proof and associate it with relevant work.</p>
        </div>
      </div>
      <TestimonialManager
        testimonials={testimonials.map(({ client, ...item }) => ({
          ...item,
          clientName: client?.name ?? "Independent client",
        }))}
        projects={projects.map((project) => ({ id: project.id, label: project.title }))}
        photos={photos.map((photo) => ({ id: photo.id, label: photo.fileName }))}
      />
    </main>
  );
}
