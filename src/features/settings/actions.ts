...
export async function updateSiteSettings(_state: SiteSettingsState, formData: FormData): Promise<SiteSettingsState> {
  ...
  await getDb().$transaction([getDb().siteSetting.upsert({ where: { key: "site" }, update: { value: parsed.data }, create: { key: "site", value: parsed.data } }), getDb().auditLog.create({ data: { actorId: session.user.id, action: "SITE_SETTINGS_UPDATED", entityType: "SiteSetting", entityId: "site" } })]);
