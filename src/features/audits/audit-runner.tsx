...
export function AuditRunner({
  ...
  const [testedUrl, setTestedUrl] = useState(
    state.values?.testedUrl ?? selectedProject?.liveUrl ?? selectedProject?.demoUrl ?? "",
  );
  ...
  return (
    <form action={action} className="admin-audit-form" noValidate>
      <div className="admin-form-grid">
        <label htmlFor="audit-project">
          Published project
          <select
            id="audit-project"
            name="projectId"
        ...
        <label htmlFor="audit-strategy">
          Test strategy
          <select
            defaultValue={state.values?.strategy ?? "MOBILE"}
            id="audit-strategy"
            name="strategy"
          >
        ...
        <label className="admin-form-wide" htmlFor="audit-url">
          Public URL
          <input
            id="audit-url"
            name="testedUrl"
            onChange={(event) => setTestedUrl(event.target.value)}
            placeholder="https://example.com"
            type="url"
            value={testedUrl}
          />
          {state.errors?.testedUrl ? (
            <span>{state.errors.testedUrl[0]}</span>
          ) : (
            <small>Use the exact production page whose evidence should be recorded.</small>
          )}
        </label>
        {state.message ? (
          <p
      ...
      <div className="admin-form-actions">
        <button
          ...
          type="submit"
        >
          {pending ? "Running PageSpeed audit…" : "Run verified audit"}
        </button>
      </div>
    </form>
  );
