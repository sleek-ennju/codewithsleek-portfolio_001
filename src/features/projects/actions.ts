...
function databaseUnavailableState(values: ProjectFormSnapshot): ProjectFormState {
  return {
    message:
      "The database is temporarily unavailable. Your form values are still here—please check your connection and try saving again.",
    values,
...
function snapshot(formData: FormData): ProjectFormSnapshot {
  ...
  return {
    ...
    repositoryVisible: checkbox(formData, "repositoryVisible"),
    ...
    technologies: formData.getAll("technologies").map(String).join(", "),
    metrics: String(formData.get("metrics") ?? ""),
    cardImageId: String(formData.get("cardImageId") ?? ""),
    coverImageId: String(formData.get("coverImageId") ?? ""),
    socialImageId: String(formData.get("socialImageId") ?? ""),
    storyOverviewImageId: String(formData.get("storyOverviewImageId") ?? ""),
    storyFeatureImageId: String(formData.get("storyFeatureImageId") ?? ""),
    storyDetailImageId: String(formData.get("storyDetailImageId") ?? ""),
...
function projectData(
  ...
  void galleryImageIds;
  void storyOverviewImageId;
  void storyFeatureImageId;
  void storyDetailImageId;
  void technologies;
  void metrics;
  return {
    ...project,
...
async function syncProjectDetails(
  ...
  // Resolve all selected technologies in one query, then bulk-write the join rows. This keeps
  // Neon interactive transactions comfortably inside their timeout window.
  const technologies = data.technologies.length
    ? await transaction.technology.findMany({
        where: { name: { in: data.technologies } },
        select: { id: true, name: true, category: true },
      })
    : [];
  if (technologies.length !== data.technologies.length)
    throw new Error("PROJECT_TECHNOLOGY_SELECTION_STALE");
  ...
  if (data.technologies.length)
    await transaction.projectTechnology.createMany({
      data: data.technologies.map((name, position) => {
        const technology = technologyByName.get(name)!;
        return { projectId, technologyId: technology.id, category: technology.category, position };
      }),
    });
  await transaction.projectMetric.deleteMany({ where: { projectId } });
  if (data.metrics.length)
    await transaction.projectMetric.createMany({
      data: data.metrics.map((metric, position) => ({
        projectId,
        label: metric.label,
        value: metric.value,
...
async function mediaExists(data: ReturnType<typeof projectFormSchema.parse>) {
  // Validate references before opening the write transaction so a slow network check cannot
  // consume the transaction's execution window.
  const ids = [
    ...new Set(
      [
        data.cardImageId,
        data.coverImageId,
        data.socialImageId,
        data.storyOverviewImageId,
        data.storyFeatureImageId,
        data.storyDetailImageId,
...
async function syncProjectImages(
  ...
  await transaction.projectImage.deleteMany({
    where: { projectId, role: { in: ["gallery", "story"] } },
  });
  const storyImageIds = [
    data.storyOverviewImageId,
    data.storyFeatureImageId,
  ...
  const storyImages = storyImageIds.flatMap((mediaId, position) =>
    mediaId ? [{ projectId, mediaId, role: "story", position }] : [],
  );
  const galleryImages = data.galleryImageIds.map((mediaId, position) => ({
    projectId,
    mediaId,
    role: "gallery",
    position,
  }));
  if (storyImages.length || galleryImages.length)
    await transaction.projectImage.createMany({ data: [...storyImages, ...galleryImages] });
}
...
export async function createProject(
  ...
  try {
    if (!(await mediaExists(parsed.data)))
      return {
        message: "One or more selected media assets no longer exist.",
        values,
        submissionId: crypto.randomUUID(),
      };
    ...
    if (error instanceof Error && error.message === "PROJECT_TECHNOLOGY_SELECTION_STALE")
      return {
        message:
          "One or more selected technologies no longer exist. Refresh the page, review the technology selection, and save again.",
        values,
        submissionId: crypto.randomUUID(),
      };
    if (typeof error === "object" && error && "code" in error && error.code === "P2028")
      return {
        message:
          "The project save took longer than the database transaction window. Nothing was committed—please try saving once more.",
        values,
        submissionId: crypto.randomUUID(),
      };
    if (isDatabaseUnavailable(error)) {
      console.error("[projects:create] Database unavailable", {
...
export async function updateProject(
  ...
  try {
    ...
    await getDb().$transaction(
      async (transaction) => {
        await transaction.project.update({
          where: { id: projectId },
          data: projectData(parsed.data, existing.publishedAt),
        });
    ...
    if (isDatabaseUnavailable(error)) {
      console.error("[projects:update] Database unavailable", {
        projectId,
        code: "code" in (error as object) ? (error as { code?: unknown }).code : undefined,
      });
      return databaseUnavailableState(values);
...
function sectionSnapshot(formData: FormData) {
  return {
    type: String(formData.get("type") ?? "RICH_TEXT"),
    title: String(formData.get("title") ?? ""),
...
function sectionContent(
  data: ReturnType<typeof projectSectionFormSchema.parse>,
  ...
  if (data.type === "CODE_SAMPLE")
    return { code: data.primary, language: data.secondary || "text" };
  if (data.type === "METRICS_GRID")
    return {
      metrics: data.primary.split("\n").map((line) => {
...
export async function createProjectSection(
  projectId: string,
  _state: ProjectSectionFormState,
  formData: FormData,
): Promise<ProjectSectionFormState> {
  const session = await requireAdmin();
  const values = sectionSnapshot(formData);
  const parsed = projectSectionFormSchema.safeParse(values);
  if (!parsed.success)
    return {
      message: "Review the highlighted section fields.",
      errors: parsed.error.flatten().fieldErrors,
      values,
      submissionId: crypto.randomUUID(),
    };
  const project = await getDb().project.findUnique({
    ...
    select: {
      slug: true,
      sections: { select: { position: true }, orderBy: { position: "desc" }, take: 1 },
    },
  });
  if (!project) return { message: "This project no longer exists." };
  await getDb().$transaction(async (transaction) => {
    const section = await transaction.projectSection.create({
      data: {
        projectId,
        type: parsed.data.type,
        title: parsed.data.title || null,
        content: sectionContent(parsed.data),
        position: (project.sections[0]?.position ?? -1) + 1,
      },
    });
    await transaction.auditLog.create({
      data: {
        actorId: session.user.id,
        action: "PROJECT_SECTION_CREATED",
        entityType: "ProjectSection",
        entityId: section.id,
        metadata: { projectId, type: parsed.data.type },
      },
    });
  });
...
export async function moveProjectSection(
  ...
  await requireAdmin();
  const sections = await getDb().projectSection.findMany({
    where: { projectId },
  ...
  await getDb().$transaction(async (transaction) => {
    ...
    for (const [position, section] of reordered.entries())
      await transaction.projectSection.update({ where: { id: section.id }, data: { position } });
  });
  const project = await getDb().project.findUnique({
    where: { id: projectId },
    select: { slug: true },
...
export async function deleteProjectSection(projectId: string, sectionId: string) {
  ...
  await getDb().$transaction([
    ...
    getDb().auditLog.create({
      data: {
        actorId: session.user.id,
        action: "PROJECT_SECTION_DELETED",
        entityType: "ProjectSection",
        entityId: sectionId,
        metadata: { projectId },
      },
  ...
  const remaining = await getDb().projectSection.findMany({
    where: { projectId },
    orderBy: { position: "asc" },
    select: { id: true },
  });
  await getDb().$transaction(
    remaining.map((item, position) =>
      getDb().projectSection.update({ where: { id: item.id }, data: { position } }),
    ),
  );
  revalidateProjectSectionPaths(projectId, section.project.slug);
}
