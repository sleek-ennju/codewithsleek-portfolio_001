...
export const projectFormSchema = z
  .object({
    ...
    industries: z
      ...
      .pipe(z.array(z.string().max(60)).max(8)),
    ...
    metrics: z
      ...
      .transform((value, context) =>
        value
          ...
          .filter(Boolean)
          .map((line, index) => {
            const [label = "", metricValue = "", unit = ""] = line
              .split("|")
              .map((part) => part.trim());
            if (!label || !metricValue)
              context.addIssue({
                code: "custom",
                message: `Metric line ${index + 1} must use Label | Value | Unit.`,
              });
  ...
  .superRefine((project, context) => {
    if (project.repositoryVisible && !project.repositoryUrl) {
      context.addIssue({
        code: "custom",
        path: ["repositoryUrl"],
        message: "Add a repository URL before making it public.",
      });
    }
    if (project.status === "PUBLISHED") {
      const required: Array<[keyof typeof project, string]> = [
        ["overview", "Add an overview before publishing."],
        ["problem", "Describe the problem before publishing."],
        ["solutions", "Describe the solution before publishing."],
        ["outcome", "Describe the outcome before publishing."],
        ["cardImageId", "Choose a card image before publishing."],
        ["coverImageId", "Choose a cover image before publishing."],
      ];
      for (const [path, message] of required)
        if (!project[path]) context.addIssue({ code: "custom", path: [path], message });
    }
    const storyImages = [
      project.storyOverviewImageId,
      project.storyFeatureImageId,
      project.storyDetailImageId,
    ].filter(Boolean);
    if (new Set(storyImages).size !== storyImages.length) {
      context.addIssue({
        code: "custom",
        path: ["storyOverviewImageId"],
        message: "Choose three different images for the landing-page story.",
      });
    }
  });
...
export type ProjectFormSnapshot = {
  ...
  storyOverviewImageId: string;
  storyFeatureImageId: string;
  storyDetailImageId: string;
...
export const projectSectionFormSchema = z
  ...
  .superRefine((section, context) => {
    if (section.type === "TWO_COLUMN" && !section.secondary)
      context.addIssue({
        code: "custom",
        path: ["secondary"],
        message: "Add content for the second column.",
      });
...
export type ProjectSectionFormState = {
  message?: string;
  errors?: Record<string, string[]>;
  values?: { type: string; title: string; primary: string; secondary: string };
  submissionId?: string;
};
