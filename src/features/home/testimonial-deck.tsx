...
function DeckArrow({ direction }: { direction: "previous" | "next" }) {
  return (
    <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24">
      <path d={direction === "previous" ? "M19 12H5m6-6-6 6 6 6" : "M5 12h14m-6-6 6 6-6 6"} />
    </svg>
  );
}
...
export function TestimonialDeck({ testimonials }: { testimonials: HomeTestimonial[] }) {
  ...
  const initials = active.authorName
    .split(/\s+/)
    .map((part) => part[0])
    .join("")
    .slice(0, 2)
  ...
  return (
    <div className="testimonial-deck">
      <div className="testimonial-stage">
        ...
        <div
          className="testimonial-shadow-card testimonial-shadow-card-middle"
          aria-hidden="true"
        />
        <figure
        ...
        >
          ...
          <blockquote className={active.quote.length > 260 ? "is-long" : undefined}>
            “{active.quote}”
          </blockquote>
          <figcaption>
            <div className="testimonial-avatar">
              {active.photo ? (
                <Image
                  src={active.photo.secureUrl}
                  alt={active.photo.altText || `${active.authorName} portrait`}
                  fill
                  sizes="64px"
                />
              ) : (
            ...
            <div>
              ...
              <span>
                {active.authorRole}
                {active.client ? ` · ${active.client.name}` : ""}
              </span>
            </div>
            {active.project && (
              <Link href={`/projects/${active.project.slug}`}>
                View project <DiagonalArrow />
              </Link>
            )}
          </figcaption>
        </figure>
        ...
        >
          {testimonials.map((testimonial, index) => (
            <button
              ...
              aria-label={`Show testimonial ${index + 1} from ${testimonial.authorName}`}
              aria-current={index === activeIndex ? "true" : undefined}
              onClick={() => show(index, index > activeIndex ? "next" : "previous")}
            >
              <span />
            </button>
          ))}
        ...
        {testimonials.length > 1 && (
          <div className="testimonial-arrows">
            <button
              ...
              onClick={() => show(activeIndex - 1, "previous")}
            >
              <DeckArrow direction="previous" />
              <span>Previous</span>
            </button>
            <button
              type="button"
