  ...
  return (
    <div className="project-art">
      <div className="project-gallery-media" aria-live="polite">
        {visuals.map((visual, index) => (
          <Image
            ...
            data-visual-frame={visual.frame}
            key={`${visual.secureUrl}-${index}`}
            src={visual.secureUrl}
            alt={
              index === activeIndex
                ? (visual.altText ?? `${projectTitle} project view ${index + 1}`)
                : ""
            }
            fill
      ...
      <Link
        ...
        href={projectHref}
        aria-label={`Read the ${projectTitle} case study`}
      >
        <i>
          <DiagonalArrow />
        </i>
      ...
      {visuals.length > 1 && (
        <div className="project-gallery-controls" aria-label={`${projectTitle} gallery controls`}>
          <button
            type="button"
          ...
          <button type="button" onClick={showNext} aria-label={`Show next ${projectTitle} image`}>
            <svg aria-hidden="true" viewBox="0 0 20 20">
