...
export async function getDashboardOverview() {
  const db = getDb();
  const [projects, drafts, published, archived, featured, media, audits, testimonials, recentActivity] = await Promise.all([
    db.project.count(),
  ...
  return { projects, drafts, published, archived, featured, media, audits, testimonials, recentActivity };
}
