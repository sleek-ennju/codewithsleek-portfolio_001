...
export function uploadPortfolioImage(buffer: Buffer) {
  return new Promise<UploadApiResponse>((resolve, reject) => {
    const stream = client().uploader.upload_stream({ resource_type: "image", folder: "codewithsleek/portfolio", use_filename: true, unique_filename: true, overwrite: false }, (error, result) => {
